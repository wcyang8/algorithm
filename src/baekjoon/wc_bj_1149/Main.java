 ...
 * 4. 조건 만족
 * 	4.1. 1,2번 집은 색이 다름.
 * 	4.2. N번 집의 색은 N-1번 집의 색과 다름.
 * 	4.3. i번 집의 색은 i-1번, i+1번 집의 색과 다름.
 * 5. 각 집을 각 색으로 칠하는 비용이 존재.
 * 6. 모든 집을 칠하는 비용의 최솟값
 * 
 * 유의
 * 1. N은 2~1000
 ...
 * 
 * 값
 * 1. 1000개의 집을 1000으로 칠해도 100만이므로 int 사용
 * 
 * 풀이
...
public class Main {
	...
	public static void main(String[] args) throws Exception{
		...
		for(int i = 2; i < N; i++) {
			int second = 0;
			for(int k = 0; k < 3; k++) {
				if(cost[i] > colorCost[i][k]) {
					last[i] = k;
					second = last[i];
					cost[i] = colorCost[1][i] + cost[i-1];
				}
			}
			if(last[i] == last[i-1]) {
				int already = 0;
				for(int k = 0; k < 3; k++) {
					if(k == last[i] || k == last[i-2]) continue;
					already = k;
				}
				// i-1번째 집의 2번째 + i번째 집의 1번째 vs i-1번째 집의 1번째 + i번째 집의 2번째
				if(cost[i-2] + colorCost[i-1][already] + colorCost[i][last[i]] <= cost[i-1] + colorCost[i][second]) {
